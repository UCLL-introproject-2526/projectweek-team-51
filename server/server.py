...
class Player:
	def __init__(self, player_id: int, team_index: int, x: float, y: float):
		...
		# latest aiming angle in degrees (0 -> right, counter-clockwise)
	...
	def update_inputs(self, up: bool, down: bool, left: bool, right: bool, shoot: bool, angle_deg: Optional[float] = None, aim_x: Optional[float] = None, aim_y: Optional[float] = None):
		...
		if angle_deg is not None:
			...
			try:
				dx = float(aim_x) - self.x
				dy = float(aim_y) - self.y
	...
	def compute_shoot_direction(self) -> Tuple[float, float]:
		# Direction from aim angle in degrees (0 -> right, ccw; convert screen coords)
		rad = math.radians(self.aim_angle_deg)
		dir_x = math.cos(rad)
		dir_y = -math.sin(rad)  # invert because screen y increases downward
...
class GameServer:
		...
		with self.game_state.lock:
			...
			for player in self.game_state.players.values():
				...
				if not player.alive:
					...
					# Skip movement and shooting while dead
					continue
				...
				if player.input_shoot and player.try_consume_shot(now):
					...
					if closest_victim is not None:
						...
						self.game_state.team_scores[player.team_index] = self.game_state.team_scores.get(player.team_index, 0) + 1
	...
	def _broadcast_state(self, now: float):
		...
		# Broadcast to all clients; include per-connection your_id
		broken_ids: List[int] = []
		with self._clients_lock:
	...
	def _ray_hits_circle(ox: float, oy: float, dx: float, dy: float, cx: float, cy: float, radius: float) -> Optional[float]:
		"""
		Return distance t >= 0 along ray origin (ox,oy) + t*(dx,dy) to the first intersection
		with a circle at (cx,cy) with given radius. None if no intersection in front of the ray.
		Ray direction (dx,dy) is assumed normalized.
		"""
		fx = cx - ox
		fy = cy - oy
		t = fx * dx + fy * dy
		...
		if dist_sq > (radius * radius):
			return None
		thickness = math.sqrt(max(0.0, (radius * radius) - dist_sq))
		hit_t = t - thickness
		if hit_t < 0:
			hit_t = t + thickness
		return hit_t if hit_t >= 0 else None
