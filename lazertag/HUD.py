...
class hud:
    ...
    def __init__(self):
        ...
        # Team score display for laser tag - MOVED TO TOP OF SCREEN
        self.team_score_text = TEXT.Text(int(SETTINGS.canvas_actual_width/2.5), 15, 'TEAM SCORE', SETTINGS.WHITE, 'DUGAFONT.ttf', 48)
    ...
    def render(self, canvas):
        ...
        # LASER TAG - Show ammo as "25/." for laser rifle (infinite ammo after 25 rounds)
        if SETTINGS.current_gun:
            if SETTINGS.current_gun.ammo_type is None:  # Laser rifle has no ammo type
                self.text[2].update_string('%s / .' % SETTINGS.current_gun.current_mag)
            elif SETTINGS.current_gun.ammo_type:
        ...
        # Draw team scores for laser tag
        green_score = SETTINGS.team_kills['green']
        orange_score = SETTINGS.team_kills['orange']
        self.team_score_text.update_string('GREEN %d  -  %d ORANGE' % (green_score, orange_score))
        self.team_score_text.draw(canvas)
