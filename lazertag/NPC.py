...
class Npc:
    ...
    def __init__(self, stats, sounds, texture, team='orange'):
        #Technical settings
        ...
        self.team = team  # 'green' or 'orange'
        self.spawn_pos = stats['pos'][:]  # Store original spawn position for respawning
        self.respawn_timer = 0  # Timer for respawn delay

        # Override texture path based on team for laser tag
        if self.team == 'green':
            texture = os.path.join('graphics', 'npc', 'green_team_player.png')
        else:  # orange team
            texture = os.path.join('graphics', 'npc', 'orange_team_player.png')
        ...
        # LASER TAG - Skip ammo check if weapon has no ammo type (infinite ammo)
        if SETTINGS.inventory['primary'] and SETTINGS.inventory['primary'].ammo_type and SETTINGS.held_ammo[SETTINGS.inventory['primary'].ammo_type] >= SETTINGS.max_ammo[SETTINGS.inventory['primary'].ammo_type]:
    ...
    def think(self):
        ...
        # LASER TAG - Handle NPC respawning
        if self.dead:
            self.respawn_timer += SETTINGS.dt
            if self.respawn_timer >= 3:  # Respawn after 3 seconds
                # Reset NPC state
                self.dead = False
                self.solid = True
                ...
                self.map_pos = self.spawn_pos[:]
                self.pos = [self.spawn_pos[0]*SETTINGS.tile_size, self.spawn_pos[1]*SETTINGS.tile_size]
                self.rect.center = (self.pos[0] + SETTINGS.tile_size/2, self.pos[1] + SETTINGS.tile_size/2)
                self.real_x = self.rect.x
                self.real_y = self.rect.y
                print(f"[LASER TAG] {self.team.upper()} team NPC respawned at spawn point")
        ...
        if not self.dead and self.health > 0 and not SETTINGS.player_states['dead']:
            ...
            if self.dist and self.dist <= SETTINGS.render * SETTINGS.tile_size * 1.2:
                ...
                #HOSTILE
                    ...
                    # Team-aware AI: Only attack if player is on opposite team
                    is_enemy = (self.team != SETTINGS.player_team)

                    if self.state == 'idle':
                        self.idle()
                        if not SETTINGS.ignore_player and is_enemy:
                        ...
                        if self.player_in_view and not SETTINGS.ignore_player and is_enemy and self.detect_player():
                            self.path = []
                            SOUND.play_sound(self.sounds['spot'], self.dist)
                            self.state = 'attacking'
                        elif self.dist <= SETTINGS.tile_size / 2 and not SETTINGS.ignore_player and is_enemy:
                            state = 'attacking'
                        else:
                            self.move()

                    elif self.state == 'attacking':
                        # Only continue attacking if still an enemy
                        if is_enemy:
                            self.attack()
                        else:
                            # Return to previous state if attacking a teammate
                            self.state = self.OG_state
    ...
    def animate(self, animation):
        ...
        #die animation
            ...
            if self.timer >= self.frame_interval and self.current_frame < len(self.die_texture)-1:
                ...
                # Track team kills for laser tag
                # If this NPC is on the opposite team from the player, player's team gets a kill
                if self.team != SETTINGS.player_team:
                    SETTINGS.team_kills[SETTINGS.player_team] += 1
                    print(f"[LASER TAG] {SETTINGS.player_team.upper()} team scored! Score: GREEN {SETTINGS.team_kills['green']} - ORANGE {SETTINGS.team_kills['orange']}")
