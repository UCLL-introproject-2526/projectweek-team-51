...
class Gun:
    ...
    def __init__(self, textures, stats, sounds, aim_pos):#  damage, spread, hit_percent, magazine_size, reload_speed, zoom, aim_pos):
        ...
        # LASER TAG - Add safety timer to prevent weapon freeze
        self.stuck_timer = 0
        self.last_shoot_busy = False
        self.last_reload_busy = False
    ...
    def reload_animation(self):
        ...
        can_reload = (self.ammo_type is None) or (SETTINGS.held_ammo[self.ammo_type] > 0) or SETTINGS.unlimited_ammo
        ...
        if can_reload:
            ...
            if (self.current_img == self.reload[3] and self.timer > self.rlspeed) or (self.current_img != self.reload[-1] and self.current_img != self.reload[3] and self.timer >= 0.15):
                ...
                if self.ammo_type is None:
                    # LASER TAG - Infinite ammo, just refill magazine
                    self.current_mag = self.mag_size
                elif not SETTINGS.unlimited_ammo:
    ...
    def draw(self, canvas):
        ...
        if not SETTINGS.player_states['dead']:
            ...
            # LASER TAG - Safety mechanism to prevent weapon freeze
            # If weapon is stuck in shoot_busy or reload_busy for more than 3 seconds, reset it
            if self.shoot_busy or self.reload_busy:
                self.stuck_timer += SETTINGS.dt
                if self.stuck_timer > 3.0:
                    # Force reset the weapon
                    self.shoot_busy = False
                    self.reload_busy = False
                    self.aim_busy = False
                    self.current_img = self.aim[0]
                    self.stuck_timer = 0
                    print("[LASER TAG] Weapon unstuck - forced reset")
            else:
                self.stuck_timer = 0
